...
{ 
    ...
    'picture_filename' : 'mypic_testfile_csv',
...
{ 
    ...
    'picture_filename' : 'mypic_local_dataframe',
    #'csv_filename' : 'test_missing_files.csv'
}
)
...
df['commitA'] = np.random.randint(0,3,size=(len(date_rng)))


# add a data element
ts = pd.to_datetime("2019-03-07 18:47:05.487", format="%Y-%m-%d %H:%M:%S.%f")
df =df.append({'date' : ts , 'commitA' : 2},ignore_index=True)
...
{ 
    ...
    'picture_filename' : 'mypic_external_dataframe',
    #'csv_filename' : 'test_missing_files.csv',
    'DataFrame' : df
}
)
